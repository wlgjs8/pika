...
def _hub_of(port):
    """RealSense physical_port / by-path 문자열에서 USB 허브 번호(예: '10') 추출.

    RealSense physical_port: '/sys/.../usb1/1-10/1-10.2/...'  -> '10'
    by-path:                 'pci-...-usb-0:10.1:1.0-video...' -> '10'
    """
    if not port:
        return None
    m = re.search(r"(?:^|[/-])1-(\d+)\.", port) or re.search(r"usb-0:(\d+)\.", port)
