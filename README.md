...
재생 기본값은 **30 fps(= 수집 레이트)로 실시간**입니다. 상태줄 둘째 줄에 재생 상태를
모두 표시하므로 터미널을 볼 필요가 없습니다.

```
[재생] 배속 x1.00   rec 30.0Hz   실제 29.8fps      ← 재생 중(호박색)
[정지] 배속 x2.25   rec 30.0Hz   실제 --           ← 정지 중(회색)
```

`배속`은 `+`/`-` 로 바꾼 배수, `rec`는 에피소드의 실제 수집 레이트(`effective_hz`),
...
`+`/`-` 배속은 프레임을 건너뛰어 맞추므로 x2 에서도 실제 시간축 2배가 됩니다.
`--fps` 로 기준 레이트를 바꿀 수 있습니다.
